...
class Entity {
            ...
            for (let i = 0; i < entityBinding.batch.length; i++) {
                ...
                try {
                    ...
                    if (!returnState.results[i]) {
                        ...
                        returnState.results[i] = new OperationResult_1.OperationResult(false, elapsedMs);
                    }
                }
                catch (error) {
                    const elapsedMs = this.computeElapsedMilliseconds(startTime);
                    returnState.results[i] = new OperationResult_1.OperationResult(true, elapsedMs, JSON.stringify(error));
                }
            }
            return returnState;
        });
    ...
    return(returnState, startTime, result) {
        ...
        returnState.results.push(new OperationResult_1.OperationResult(false, this.computeElapsedMilliseconds(startTime), JSON.stringify(result)));
    }
    ...
    signalEntity(returnState, entity, operationName, operationInput) {
        returnState.signals.push(new Signal_1.Signal(entity, operationName, operationInput ? JSON.stringify(operationInput) : ""));
    }
